...
async def setWelcomeHandler(c, m):
    ...
    asyncio.create_task(welcomeFunc(c, m, k))
...
async def welcomeRespons(c: Client, m: Message):
    ...
    k = cached_redis_get(f'{Dev_Zaid}:botkey', ttl=120)
    channel = (
        cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) if cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) else "eFFb0t"
    )
    print("member")
    if not cached_redis_get(f"{m.chat.id}:disableWelcome:{Dev_Zaid}", ttl=60) and m.new_chat_members:
        if not r.get(f"{m.chat.id}:CustomWelcome:{Dev_Zaid}"):
        ...
        for me in m.new_chat_members:
            if not me.id == int(Dev_Zaid):
                if r.get(f"{m.chat.id}:enableVerify:{Dev_Zaid}") and not pre_pls(
                    ...
                    return
                photo = None
                if not cached_redis_get(f"{m.chat.id}:disableWelcomep:{Dev_Zaid}", ttl=60) and me.photo:
...
"""
async def welcomeRespons(c,m):
   if not cached_redis_get(f'{m.chat.id}:enable:{Dev_Zaid}', ttl=30):  return
   k = cached_redis_get(f'{Dev_Zaid}:botkey', ttl=120)
   channel = cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) if cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) else 'Y88F8'
   print("member")
   if not cached_redis_get(f'{m.chat.id}:disableWelcome:{Dev_Zaid}', ttl=60) and m.new_chat_members:
     if not r.get(f'{m.chat.id}:CustomWelcome:{Dev_Zaid}'):
        welcome = default_welcome
