...
from akinator.async_aki import Akinator as AkntClient
import asyncio
...
async def gamesHandler(c,m):
    ...
    asyncio.create_task(gamesFunc(c,m,k,channel))

@Client.on_message(filters.dice & filters.group, group=45)
async def diceFunc(c,m):
   if cached_redis_get(f'{m.chat.id}:disableGames:{Dev_Zaid}', ttl=60):  return False
   if m.dice.emoji == "🎲":
     k = cached_redis_get(f'{Dev_Zaid}:botkey', ttl=120)
     if m.dice.value == 6:
...
async def gamesFunc(c,m,k,channel):
   ...
   if cached_redis_get(f'{m.chat.id}:disableGames:{Dev_Zaid}', ttl=60):  return
   ...
   if r.get(f'{m.from_user.id}:toTrans:{m.chat.id}{Dev_Zaid}'):
      ...
      floos_to_trans = int(r.get(f'{m.from_user.id}:toTrans:{m.chat.id}{Dev_Zaid}') or 0)
      r.delete(f'{m.from_user.id}:toTrans:{m.chat.id}{Dev_Zaid}')
   ...
   if r.get(f'{m.from_user.id}:createBank:{m.chat.id}'):
     ...
     if text == 'عبد الفتاح السيسي':
        ...
        if r.get(f'DevGroup:{Dev_Zaid}'):
          return c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),
     ...
     await m.reply(f'• وسوينا لك حساب في بنك {text}\n\n{k} رقم حسابك ↢ ( `{id}` )\n{k} نوع البطاقة ↢ ( {card} )\n{k} فلوسك ↢ ( {fff} ريال 💸 )')
     if r.get(f'DevGroup:{Dev_Zaid}'):
         c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),
           f' ⟨ {m.from_user.mention} ⟩\n{k} سوى حساب بالبنك\n{k} رقم حسابه ( `{id}` )')
...
''')
   ...
   if text.startswith('بيع فلوسي ') and len(text.split()) == 3 and re.findall('[0-9]+', text):
     if not r.get(f'{m.from_user.id}:Floos'):
        ...
        get = int(r.get(f'{m.chat.id}:TotalMsgs:{m.from_user.id}{Dev_Zaid}') or 0)
   ...
   if text.startswith('كشط ') and len(text.split()) == 2:
     ...
     if r.get(f'DevGroup:{Dev_Zaid}'):
       ...
       c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),alert)
   ...
   if text.startswith('زواج ') and re.findall('[0-9]+', text) and m.reply_to_message and m.reply_to_message.from_user and len(text.split()) == 2:
     ...
     if r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}'):
       getUser = c.get_users(int(r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}') or 0))
     ...
     if r.get(f'{m.from_user.id}:marriedWomen:{m.chat.id}{Dev_Zaid}'):
       getUser = c.get_users(int(r.get(f'{m.from_user.id}:marriedWomen:{m.chat.id}{Dev_Zaid}') or 0))
       mention = getUser.mention
       return await m.reply(f'「 {mention} 」 \n{k} تعال زوجتك بتخونك')
     if not r.get(f'{m.from_user.id}:Floos'):
       floos_from_user = 0
...
''')
   ...
   if text== 'طلاق' and r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}'):
     getUser = c.get_users(int(r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}') or 0))
     floos = int(r.get(f'{m.from_user.id}:MARRYMONEY:{m.chat.id}{Dev_Zaid}') or 0)
     r.srem(f'{m.chat.id}:zwag:{Dev_Zaid}', f'{getUser.id}--{m.from_user.id}&&floos={floos}')
     if not r.get(f'{getUser.id}:Floos'):
       floos_from_whife = 0
     else:
       floos_from_whife = int(r.get(f'{getUser.id}:Floos') or 0)
   ...
   if text== 'خلع' and r.get(f'{m.from_user.id}:marriedWomen:{m.chat.id}{Dev_Zaid}'):
     ...
     if not r.get(f'{getUser.id}:Floos'):
       ...
       floos_from_has = int(r.get(f'{getUser.id}:Floos') or 0)
...
''')
   ...
   if text == 'تم' and r.get(f'{m.chat.id}:AHKAMGAME:{Dev_Zaid}') and m.from_user.id == int(r.get(f'{m.chat.id}:AHKAMGAME:{Dev_Zaid}') or 0):
...
''')
   ...
   if text == 'تم' and r.get(f'{m.chat.id}:ROLETGAME:{Dev_Zaid}') and m.from_user.id == int(r.get(f'{m.chat.id}:ROLETGAME:{Dev_Zaid}') or 0):
...
async def akinatorHandler(c,m):
   channel = cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) if cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) else 'yqyqy66'
   rep_btn = InlineKeyboardMarkup([[InlineKeyboardButton('🧚‍♀️', url=f't.me/{channel}')]])
   aki_markup = InlineKeyboardMarkup([
       [
   ...
   if m.data == f'start_aki:{m.from_user.id}':
    await m.edit_message_text("⇜ جاري بدء اللعبة...", reply_markup=rep_btn)
    aki = AkntClient()
    q = await asyncio.to_thread(aki.start_game, language="ar")
    users_demon.update({m.from_user.id: [aki, q]})
    return await m.edit_message_text(users_demon[m.from_user.id][1], reply_markup=aki_markup)

   for ans_key, ans_val in [('n', 'n'), ('y', 'y'), ('p', 'p')]:
    if m.data == f'aki_c:{ans_key}++{m.from_user.id}':
      users_demon[m.from_user.id][1] = await asyncio.to_thread(
          users_demon[m.from_user.id][0].answer, ans_val
      )
      if users_demon[m.from_user.id][0].progression >= 65:
          await asyncio.to_thread(users_demon[m.from_user.id][0].win)
          str_to_send = users_demon[m.from_user.id][0].first_guess
          print(str_to_send)
          await m.message.delete()
          try:
              await c.send_photo(m.message.chat.id, str_to_send['absolute_picture_path'],
                  caption=f"{str_to_send['name']} - {str_to_send['description']}", reply_markup=rep_btn)
          except:
              await c.send_message(m.message.chat.id,
                  f"{str_to_send['name']} - {str_to_send['description']}", reply_markup=rep_btn)
          del users_demon[m.from_user.id]
      else:
          return await m.edit_message_text(users_demon[m.from_user.id][1], reply_markup=aki_markup)
