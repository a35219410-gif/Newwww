...
async def ranksCommandsHandler(c,m):
   ...
   asyncio.create_task(ranks_reply_promote(c,m,k))
...
async def ranks_reply_promote(c,m,k):
    ...
    if text == 'تعطيل الرفع':
      if not owner_pls(m.from_user.id, m.chat.id):
        ...
        if cached_redis_get(f'{m.chat.id}:disableRanks:{Dev_Zaid}', ttl=60):
    ...
    if text == 'تفعيل الرفع':
      if not owner_pls(m.from_user.id, m.chat.id):
        ...
        if not cached_redis_get(f'{m.chat.id}:disableRanks:{Dev_Zaid}', ttl=60):
    ...
    if cached_redis_get(f'{m.chat.id}:disableRanks:{Dev_Zaid}', ttl=60):  return
...
def ranks_reply_demote(c,m,k):
    ...
    if text.startswith('تنزيل الكل '):
       ...
       if dev2_pls(m.from_user.id, m.chat.id):
          rank = get_rank(id,cid)
          if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner') or 0) and not id in [6168217372]:
              await m.reply(f'「 {mention} 」\n{k} نزلته من {rank} \n☆')
              r.delete(f'{id}:rankDEV:{Dev_Zaid}')
              r.srem(f'{Dev_Zaid}DEV', id)
              r.delete(f'{cid}:rankGOWNER:{id}{Dev_Zaid}')
              ...
              r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
              return
          if id in [6168217372, 5117901887] or id == int(r.get(f'{Dev_Zaid}botowner') or 0):
              return await m.reply(f'{k} رتبته اعلى منك')
          else:
              return await m.reply(f'{k} ماله رتبة')

       if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner') or 0) and not id in [6168217372] and not r.get(
           ...
           r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
           return
       if id in [6168217372, 5117901887] or id == int(r.get(f'{Dev_Zaid}botowner') or 0) or not r.get(
               f'{id}:rankDEV2:{Dev_Zaid}'):
           return await m.reply(f'{k} رتبته اعلى منك')
       else:
           return await m.reply(f'{k} ماله رتبة')
    ...
    if text == 'تنزيل الكل' and m.reply_to_message and m.reply_to_message.from_user:
       ...
       if gowner_pls(m.from_user.id, m.chat.id):
           rank = get_rank(id, cid)
           if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner') or 0) and not id in [
       ...
       if admin_pls(m.from_user.id, m.chat.id):
           ...
           if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner') or 0) and not id in [
               ...
               r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
               return
           if id in [6168217372, 5117901887] or id == int(r.get(f'{Dev_Zaid}botowner') or 0) or r.get(
                   f'{id}:rankDEV2:{Dev_Zaid}') or r.get(f'{id}:rankDEV:{Dev_Zaid}') or r.get(
