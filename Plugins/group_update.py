...
from config import *
from .moderation import list_UwU
...
def kick_from_group(app: Client, m: Update, _, __):
   try:
      ...
      if r.get(f'DevGroup:{Dev_Zaid}'):
        app.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text,disable_web_page_preview=True)
      else:
...
async def globalHandler(c,m):
   asyncio.create_task(global_filter(c,m))
...
async def filtersHandler(c,m):
   asyncio.create_task(get_filter(c,m))
...
async def get_filter(c,m):
   ...
   if r.get(f'{text}:filterMEM:{Dev_Zaid}{m.chat.id}') and not r.get(f'{m.chat.id}:lock_filterMEM:{Dev_Zaid}'):
     id = int(r.get(f'{text}:filterMEM:{Dev_Zaid}{m.chat.id}') or 0)
     get = c.get_chat(id)
     ...
     if not get.photo:
       ...
       if r.get(f"{hash}:{id}"):
         return await m.reply_animation(r.get(f"{hash}:{id}"), caption=cap, reply_markup=reply_markup) 
       video = photo.video_sizes[0] if photo.video_sizes else None
       if video:
         file = BytesIO()
         for byte in c.stream_media(
...
async def globalRandomupdate(c,m):
   asyncio.create_task(get_rngp(c,m))
...
async def randomfiltersHandler(c,m):
   asyncio.create_task(get_rn_filter(c,m))
...
def kick_from_gp(c,m):
   if m.left_chat_member.id == int(Dev_Zaid):
        ...
        text += '\n\n☆'
        if r.get(f'DevGroup:{Dev_Zaid}'):
          c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text,disable_web_page_preview=True)
        else:
          for dev in get_devs_br():
                 try:
  ...
  try:
    if m.new_chat_member.status == ChatMemberStatus.MEMBER:
       if m.new_chat_member.user.id == c.me.id:
         if cached_redis_get(f'{m.chat.id}:enable:{Dev_Zaid}', ttl=30):
             text = f'{k} من「 {m.from_user.mention} 」\n'
             ...
             text += '\n\n☆'
             if r.get(f'DevGroup:{Dev_Zaid}'):
                   c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text)
             else:
               for dev in get_devs_br():
                 try:
                    c.send_message(int(dev), text, disable_web_page_preview=True)
       ...
       if m.new_chat_member.user.id == c.me.id:
          ...
          if not cached_redis_get(f'{m.chat.id}:enable:{Dev_Zaid}', ttl=30):
             ...
             if priv.can_manage_chat and priv.can_delete_messages and priv.can_restrict_members and priv.can_pin_messages and priv.can_invite_users:
                ...
                text += '\n\n☆'
                if r.get(f'DevGroup:{Dev_Zaid}'):
                   c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text,reply_markup=reply_markup,disable_web_page_preview=True)
                else:
                  for dev in get_devs_br():
                    try:
    ...
    if not priv.can_manage_chat or not priv.can_delete_messages or not priv.can_pin_messages or not priv.can_invite_users:
      return await m.reply(f'{k} عطيني كل الصلاحيات بعدين ارسل تفعيل')
        ...
        if get.invite_link:
           ...
           reply_markup=None
        if r.get(f'DevGroup:{Dev_Zaid}'):
                   await c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text,reply_markup=reply_markup,disable_web_page_preview=True)
        else:
               for dev in get_devs_br():
                 try:
  ...
  if text == 'تعطيل':
    if not (await m.chat.get_member(m.from_user.id)).status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] and not owner_pls(m.from_user.id,m.chat.id):
      ...
      if not cached_redis_get(f'{m.chat.id}:enable:{Dev_Zaid}', ttl=30):
        ...
        text += '\n\n☆'
        if r.get(f'DevGroup:{Dev_Zaid}'):
                   await c.send_message(int(r.get(f'DevGroup:{Dev_Zaid}') or 0),text)
        else:
               for dev in get_devs_br():
                 try:
