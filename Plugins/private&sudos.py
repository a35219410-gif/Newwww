...
from helpers.utils import cached_redis_get, cache_invalidate, cache_clear_all, shell_exec
...
async def delRanksHandler(c,m):
    k = cached_redis_get(f'{Dev_Zaid}:botkey', ttl=120)
    asyncio.create_task(private_func(c,m,k))
...
'''.format(m.from_user.mention,m.from_user.id,username,len(r.smembers(f'{Dev_Zaid}:UsersList')))
       ...
       if r.get(f'DevGroup:{Dev_Zaid}'):
          c.send_message(
          int(r.get(f'DevGroup:{Dev_Zaid}') or 0),
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"تحديثات {name} 🍻", url=f't.me/{channel}')]]))
  ...
  if text.startswith(". "):
     ...
     async with aiohttp.ClientSession() as s:
         async with s.get(f"https://gptzaid.zaidbot.repl.co/1/text={text}") as resp:
             rep = await resp.text()
     try: m.reply_chat_action(ChatAction.TYPING)
     except Exception as e: print(e);pass
     await msg.edit(rep)
...
async def sudosCommandsHandler(c,m):
    k = cached_redis_get(f'{Dev_Zaid}:botkey', ttl=120)
    channel = cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) if cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120) else 'yqyqy66'
    asyncio.create_task(SudosCommandsFunc(c,m,k,r,channel))
...
async def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if text == 'قناة السورس':
     if not dev2_pls(m.from_user.id, m.chat.id):
        return
     if not cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120):
       return await m.reply(quote=True,text=f'{k} قناة السورس مو معينة')
     else:
       cha = cached_redis_get(f'{Dev_Zaid}:BotChannel', ttl=120)
   ...
   if text == 'مجموعة المطور' and m.chat.type == ChatType.PRIVATE:
     if not dev_pls(m.from_user.id,m.chat.id):
        ...
        if not r.get(f'DevGroup:{Dev_Zaid}'):
           ...
           id = int(r.get(f'DevGroup:{Dev_Zaid}') or 0)
