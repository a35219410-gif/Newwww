...
def addmsgCount(c,m):
   ...
   if not r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{m.from_user.id}'):
      ...
      get = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{m.from_user.id}') or 0)
...
def addeditedmsgCount(c,m):
   ...
   if not r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}'):
      ...
      get = int(r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}') or 0)
...
async def rankGetHandler(c,m):
   ...
   asyncio.create_task(get_my_rank(c,m,k))
...
async def get_my_rank(c,m,k):
   ...
   if text == 'معلوماتي':
      msgs = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{m.from_user.id}') or 0)
      if msgs > 50:
        tfa3l = 'شد حيلك'
      if msgs > 500:
        tfa3l = 'يجي منك'
      ...
      if not r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}'):
         ...
         edits= int(r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}') or 0)
      if not r.get(f'{m.chat.id}TotalContacts{m.from_user.id}{Dev_Zaid}'):
         contacts = 0
      else:
         contacts = int(r.get(f'{m.chat.id}TotalContacts{m.from_user.id}{Dev_Zaid}') or 0)
...
'''
   ...
   if text == 'بايو' and m.reply_to_message and m.reply_to_message.from_user:
      if cached_redis_get(f'{m.chat.id}:disableBio:{Dev_Zaid}', ttl=60):  return
   ...
   if text == 'افتاري':
     if cached_redis_get(f'{m.chat.id}:disableAV:{Dev_Zaid}', ttl=60): return False
   ...
   if text == 'مسح تكليجاتي':
      ...
      msgs = int(r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}') or 0)
   ...
   if (text == 'رسايله' or text == 'رسائلة') and m.reply_to_message and m.reply_to_message.from_user:
      msgs = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{m.reply_to_message.from_user.id}') or 0)
...
'''
   ...
   if text.startswith('كشف') and len(text.split()) > 1 and 'tg://user?id=' in m.text.html:
       ...
       try:
           ...
           id = get.user.id
           msgs = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{get.user.id}') or 0)
           ...
           try:
               ...
               name = get.first_name
               id = get.id
               msgs = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{get.id}') or 0)
               if get.user.username:
                   username = f'@{get.user.username}'
...
'''
   ...
   if text == 'تفعيل الايدي':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if not cached_redis_get(f'{m.chat.id}:disableID:{Dev_Zaid}', ttl=60):
   ...
   if text == 'تعطيل الايدي':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if cached_redis_get(f'{m.chat.id}:disableID:{Dev_Zaid}', ttl=60):
   ...
   if text == 'تفعيل افتاري':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if not cached_redis_get(f'{m.chat.id}:disableAV:{Dev_Zaid}', ttl=60):
         return await m.reply(f'{k} بواسطة ↤ {m.from_user.mention}\n{k} افتار مفعل من قبل')
       else:
   ...
   if text == 'تعطيل افتاري':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if cached_redis_get(f'{m.chat.id}:disableAV:{Dev_Zaid}', ttl=60):
         return await m.reply(f'{k} بواسطة ↤ {m.from_user.mention}\n{k} افتار معطل من قبل')
       else:
   ...
   if text == 'تعطيل الايدي بالصوره':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if cached_redis_get(f'{m.chat.id}:disableIDPHOTO:{Dev_Zaid}', ttl=60):
   ...
   if text == 'تفعيل الايدي بالصوره':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if not cached_redis_get(f'{m.chat.id}:disableIDPHOTO:{Dev_Zaid}', ttl=60):
   ...
   if (text == 'ايدي' or text.lower() == 'id') and not m.reply_to_message:
      if cached_redis_get(f'{m.chat.id}:disableID:{Dev_Zaid}', ttl=60):  return
...
{البايو}'''
      if m.from_user.usernames:
         ...
         username = 'مافي يوزر'
      rank = get_rank(m.from_user.id, m.chat.id)
      msg = int(r.get(f'{Dev_Zaid}{m.chat.id}:TotalMsgs:{m.from_user.id}') or 0)
      msgs = f"{msg}"
      iD = f'`{m.from_user.id}`'
      if not r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}'):
         edits = 0
      else:
         edit= int(r.get(f'{m.chat.id}:TotalEDMsgs:{m.from_user.id}{Dev_Zaid}') or 0)
         edits = f"{edit}"
         ...
         if m.from_user.photo:
           ...
           if video:
              ...
              if r.get(f"{hash}:{m.from_user.id}"):
                return await m.reply_animation(r.get(f"{hash}:{m.from_user.id}"), caption=text)
  ...
  for me in m.new_chat_members:
    if not m.from_user.id == me.id:
      if not r.get(f'{m.chat.id}TotalContacts{m.from_user.id}{Dev_Zaid}'):
        ...
        co = int(r.get(f'{m.chat.id}TotalContacts{m.from_user.id}{Dev_Zaid}') or 0)
